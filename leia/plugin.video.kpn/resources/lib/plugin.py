...
def vod_series(label, description, image, id, **kwargs):
    ...
    for season in seasons:
        items.append(plugin.Item(
            ...
            art = {'thumb': "{image_url}/vod/{image}/{img_size}.jpg?blurred=false".format(image_url=CONST_IMAGE_URL, image=season['image'], img_size=settings.get(key='_img_size'))},
            path = plugin.url_for(func_or_url=vod_season, label=label, id=season['id']),
        ))
...
def vod_season(label, id, **kwargs):
    ...
    for episode in season:
        items.append(plugin.Item(
            ...
            art = {'thumb': "{image_url}/vod/{image}/{img_size}.jpg?blurred=false".format(image_url=CONST_IMAGE_URL, image=episode['image'], img_size=settings.get(key='_img_size'))},
            path = plugin.url_for(func_or_url=play_video, type='vod', id=episode['id'], asset_id=episode['assetid'], duration=episode['duration'], _is_live=False),
            playable = True,
        ))
